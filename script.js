document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (e) => {
        ...
        const actions = {
            ...
            clearHistory,
            searchTools,
            summarizeText: () => summarizeText(e.target),
            convertLength: () => convertLength(e.target),
            calculate: () => calculate(e.target),
            generatePassword: () => generatePassword(e.target),
            copyPassword,
            countChars: () => countChars(e.target),
            checkURL: () => checkURL(e.target),
            convertTemp: () => convertTemp(e.target),
            convertCurrency: () => convertCurrency(e.target),
            generateQR: () => generateQR(e.target),
            compressImage: () => compressImage(e.target),
            calculateBMI: () => calculateBMI(e.target),
            convertArea: () => convertArea(e.target)
        };
